...
pub type SharedBlind = Rc<RefCell<Blind>>;
...
pub type SharedUpcomingBlinds = Rc<RefCell<UpcomingBlinds>>;
/// Container for the three blinds in an ante: Small, Big, and Boss
#[derive(Debug, Clone)]
pub struct UpcomingBlinds {
    pub small: SharedBlind,
    pub big: SharedBlind,
    pub boss: SharedBlind,
}

impl Blind {
    /// Create a new blind
    pub fn new(name: String, blind_type: BlindType, required_score: i32, reward_money: i32) -> SharedBlind {
    ...
    pub fn new_boss(name: String, required_score: i32, reward_money: i32, effect: BossEffect) -> SharedBlind {
        Rc::new(RefCell::new(Self {
            name,
    ...
    pub fn set_status(&mut self, status: BlindStatus) {
        self.status = status;
    }
}
...
impl UpcomingBlinds {
    ...
    pub fn new(small: SharedBlind, big: SharedBlind, boss: SharedBlind) -> SharedUpcomingBlinds {
        Rc::new(RefCell::new(Self { small: small.clone(), big: big.clone(), boss: boss.clone() }))
    }

    /// Get a reference to a blind by type
    pub fn get_blind(&self, blind_type: BlindType) -> Option<SharedBlind> {
        match blind_type {
            BlindType::Small => Some(self.small.clone()),
            BlindType::Big => Some(self.big.clone()),
            BlindType::Boss => Some(self.boss.clone()),
        }
    }

    /// Get the next upcoming blind (first one with Upcoming status)
    pub fn get_next_upcoming_blind(&self) -> Option<SharedBlind> {
        if self.small.borrow().status == BlindStatus::Upcoming {
            Some(self.small.clone())
        } else if self.big.borrow().status == BlindStatus::Upcoming {
            Some(self.big.clone())
        } else if self.boss.borrow().status == BlindStatus::Upcoming {
    ...
    pub fn get_active_blind(&self) -> Option<SharedBlind> {
        if self.small.borrow().status == BlindStatus::Active {
            Some(self.small.clone())
        } else if self.big.borrow().status == BlindStatus::Active {
            Some(self.big.clone())
        } else if self.boss.borrow().status == BlindStatus::Active {
            Some(self.boss.clone())
        } else {
            None
        }
...
impl BlindProcessor {
    ...
    pub fn generate_blinds(&self, ante: u32, stake: &crate::stakes::Stake) -> GameResult<SharedUpcomingBlinds> {
